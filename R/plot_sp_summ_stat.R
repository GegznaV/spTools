...
#' (*** NOT RELEVANT ***)
#'
#' Plot many summary plots of spectra (plot_sp_summ_stat)
#'
#' @param sp
#' @param vars
#' @param zones
...
plot_sp_summ_stat <- function(sp, vars, zones)
{
    ...
    for (i in 1:N) {
        ...
        # Assign variable to "gr"
        sp$gr <- sp[[,Var,drop = TRUE]] %>%
            as.factor() %>%
            droplevels()
        ...
        # band90  <- qplot_spDistrib(sp,"gr", percent = 90)  +
        #   set_ggLims(yLIM,"y") +
        #   highlight_ranges(zones)
        ...
        # Median and MAD  ------------------------------------------------------
        ggMed <- qplot_spStat(sp, "gr", median) +
            set_ggLims(yLIM,"y") +
            highlight_ranges(zones)
        ...
        suppressMessages({
            p3_BCSC_sc     <-
                qplot_sp(CSCb_sc, names.in = "gr") +
                set_ggLims(yLIM_sc,"y") +
                highlight_ranges(zones) +
        ...
        p_SeSp <- ggplot(sp_compare_gr_wl(sp, "gr",measure = "sesp"),
                         aes(color = Compared)) + highlight_ranges(zones) +
            geom_line() + theme_bw() + set_ggLims(c(.5,1),"y") +
            ggtitle("Spectrum of (Se+Sp)/2")
        #
        # p_AUC_sc <- ggplot(sp_compare_gr_wl(scSp, "gr"), aes(color = Compared)) + highlight_ranges(zones) +
        #     geom_line() + theme_bw() + ggtitle("Compare groups (AUC)\n(scaled data)")+ set_ggLims(c(.5,1),"y")
        #
        # p_SeSp_sc <- ggplot(sp_compare_gr_wl(scSp, "gr",measure = "sesp"), aes(color = Compared))+ highlight_ranges(zones) +
